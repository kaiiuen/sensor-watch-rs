...
//!
//! This is pure computation with no hardware dependency, so it lives in the
//! `core` crate and is fully unit-tested on the host.
...
mod tests {
    ...
    fn corrects_every_single_bit_position() {
        // For a range of data values, verify every single-bit error in the
        // 39-bit code word is corrected back to the original data.
        let total = HAMMING_BITS + 1;
        for data in [0u32, 1, 0xFFFF_FFFF, 0x1234_5678, 0xDEAD_BEEF] {
            let code = encode(data);
            for bit in 0..total {
                let corrupted = code ^ (1 << bit);
                let (decoded, corrected) = decode(corrupted);
                assert_eq!(decoded, data, "single-bit error at position {bit}");
                assert!(corrected, "should correct position {bit}");
            }
        }
    }
    ...
    fn detects_all_double_bit_errors() {
        // Verify that any two-bit error is detected (not silently corrected to
        // the wrong value).
        let total = HAMMING_BITS + 1;
        ...
        for b1 in 0..total {
            for b2 in (b1 + 1)..total {
                ...
                let (decoded, corrected) = decode(corrupted);
                // A double-bit error must never be reported as a clean
                // single-bit correction to a different value.
                if corrected {
                    assert_eq!(decoded, data, "double-bit error at {b1},{b2} mis-corrected");
                }
            }
        }
    }
