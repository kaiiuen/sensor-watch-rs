...
use super::components::{
    capabilities, default_profiles, BoardKind, BuildProfile, BuzzerDrive, ComponentsConfig,
    LcdVariant,
};
...
pub(crate) fn generate(
    ...
    let manifest = FirmwareManifest {
        ...
        board: BoardIdentity {
            ...
            buzzer_drive: board.buzzer_drive,
        },
        features: FeatureSelection {
...
fn board_data(board: BoardKind, revision: &str) -> Result<&'static BoardData, FirmwareInputError> {
    ...
    // Keep the capability table and the revision-gated build data in lockstep.
    if capabilities(board).buzzer_drive != data.buzzer_drive {
        return Err(FirmwareInputError::Unsupported(
            "board capability and revision profile disagree about buzzer drive".into(),
        ));
    }
    Ok(data)
}
...
mod tests {
    ...
    fn each_stock_board_generates() {
        for board in BoardKind::ALL {
            ...
            let expected_drive = if board == BoardKind::Pro {
                "9 V boosted/amplified"
            } else {
                "battery-level/unboosted"
            };
            assert!(result.files["firmware_inputs.rs"].contains(expected_drive));
            assert!(result.files["firmware_inputs.json"].contains("buzzer_drive"));
        }
    }
    #[test]
    ...
    #[test]
    fn buzzer_drive_is_revision_gated_and_unknown_revisions_fail_closed() {
        let pro = request(BoardKind::Pro);
        assert_eq!(
            capabilities(BoardKind::Pro).buzzer_drive,
            BuzzerDrive::Boosted9V
        );
        assert!(validate_request_and_sources(&pro, &super::super::build::firmware_dir()).is_ok());

        for board in [BoardKind::Green, BoardKind::RedLite, BoardKind::Blue] {
            assert_eq!(capabilities(board).buzzer_drive, BuzzerDrive::BatteryLevel);
            let mut unknown = request(board);
            unknown.revision = "unknown-pro-converter".into();
            assert!(
                validate_request_and_sources(&unknown, &super::super::build::firmware_dir())
                    .is_err()
            );
        }
        let mut mismatched = request(BoardKind::Pro);
        mismatched.revision = "OSO-SWAT-A1-05".into();
        assert!(
            validate_request_and_sources(&mismatched, &super::super::build::firmware_dir())
                .is_err()
        );
    }
