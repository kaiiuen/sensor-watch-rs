...
This project is a **from-scratch rewrite** of the original C firmware in Rust.
It is structured as a **Cargo workspace** with two crates:
...
The `core` crate contains **pure computation** (date math, bit-packing) with **no
hardware dependency**. This lets us run **unit tests on the host** (a normal
desktop), giving us *proof* the logic is correct without needing the physical
watch. The firmware crate (`sensor-watch`) depends on the PAC and can only build
...
The release binary is ~68 KB (~27% of flash). Each watch face costs ~1-3 KB.
We have ~188 KB of headroom — room for dozens more faces.

### RAM (32 KB)

- **Static state** (all faces + movement): ~300 bytes
- **Stack**: 8 KB reserved in the linker script
- **Heap**: none (zero-heap)

RAM is essentially not a constraint. All face state lives in RAM (option 2 from
the design discussion), which means **state survives face switches** and **no
flash wear from runtime state**.
...
```
1. movement::fault::check_reset_reason()  — record why we reset (watchdog/panic/power-on)
2. watch::init()                          — hardware init in dependency order:
...
3. movement::app_init()                   — load settings, init framework
4. movement::board::apply()               — apply LED polarity + buzzer voltage
5. movement::app_setup()                  — register faces, buttons, alarms
6. watch::rtc::register_tick_callback()   — 1 Hz tick
7. loop:
     a. movement::app_loop()              — react to one event
     b. watch::wdt::kick()                — watchdog heartbeat
     c. cortex_m::asm::wfi()              — enter STANDBY
```
...
- `set_date_time()` / `get_date_time()` — read/write the clock
...
`set_led_red()`, `set_led_green()`, `set_led_off()`, and `set_invert_polarity()`
(for common-anode Red/Pro boards).
...
`set_buzzer_off()`, `play_note()`, and the `Note` enum (A1-B8 + Rest).
...
**Why:** the battery voltage is read via the ADC against the internal 1.024 V
reference (you can't compare VCC to itself).

### 6.11 `watch/i2c.rs` — I2C

Uses SERCOM1 in I2C master mode. Provides `enable_i2c()`, `send()`, `receive()`,
and register helpers `write8()`, `read8()`, `read16()`, `read24()`, `read32()`.
...
- `Settings` — a 32-bit packed struct of user preferences (bit-packing with
  getters/setters). Stored in RTC backup register 0.
- `Button` — Light, Mode, Alarm
- `ButtonEvent` — Down, Up, LongPress, LongUp
- `Event` — the closed set of events: Activate, Tick, BackgroundTask, Button
- `WatchFace` — the trait every face implements
...
- `save_settings()` — persist settings to flash
...
handles long-press detection via the 128 Hz fast tick.
...
- `world_clock.rs` — time in a selected timezone
...
The core crate has 16 unit tests covering date math, settings bit-packing, and
