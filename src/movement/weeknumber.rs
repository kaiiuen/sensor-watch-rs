//! Week number clock watch face.
//!
//! Port of the C `weeknumber_clock_face.c`. Displays the weekday, day, time,
//! and the ISO week number, with a battery-low indicator and an optional
//! hourly signal. It is a pure state machine: it renders on wake and returns;
...
/// The state for the week number clock face.
pub struct WeekNumberClockFace {
    signal_enabled: bool,
...
impl WeekNumberClockFace {
    ...
    pub fn new() -> Self {
        WeekNumberClockFace::new_static()
    }
...
impl WatchFace for WeekNumberClockFace {
...
impl WeekNumberClockFace {
    /// Renders the clock and week number.
    fn draw_clock(&mut self, settings: &mut Settings) {
        ...
        let weeknumber = utility::get_weeknumber(
        ...
        if (date_time.to_reg() >> 12) == (previous >> 12) {
            // Everything before minutes is the same; update minutes + week number.
            buf[0] = b'0' + date_time.minute / 10;
            buf[1] = b'0' + date_time.minute % 10;
            buf[2] = b'0' + weeknumber / 10;
            buf[3] = b'0' + weeknumber % 10;
            watch::slcd::display_string(core::str::from_utf8(&buf[..4]).unwrap_or("  "), 6);
        } else {
            // Other stuff changed; do it all.
            ...
            buf[8] = b'0' + weeknumber / 10;
            buf[9] = b'0' + weeknumber % 10;
            watch::slcd::display_string(core::str::from_utf8(&buf[..10]).unwrap_or(""), 0);
        }
