//! Repetition minute watch face.
//!
//! Port of the C `repetition_minute_face.c`. A clock that can chime the time
//! in the traditional repetition-minute pattern (hours, quarters, minutes). It
...
/// The repetition minute face state.
pub struct RepetitionMinuteFace {
    signal_enabled: bool,
...
impl RepetitionMinuteFace {
    /// A const constructor for use in a static initializer.
    pub const fn new_static() -> Self {
        RepetitionMinuteFace {
    ...
    pub fn new() -> Self {
        RepetitionMinuteFace::new_static()
    }
...
impl WatchFace for RepetitionMinuteFace {
    ...
    fn loop_(&mut self, event: Event, settings: &mut Settings) {
        match event {
            ...
            Event::Button(Button::Light, ButtonEvent::LongUp) => {
                ...
                let quarters = date_time.minute / 15;
                let minutes = date_time.minute % 15;
                if !settings.clock_mode_24h() {
                    hours %= 12;
                    if hours == 0 {
                        hours = 12;
